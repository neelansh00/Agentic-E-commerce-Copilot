...
from app.database.metrics import available_views
...
AST_FUNCTIONS = {name.upper() for name in SQLITE_FUNCTIONS} | {
    ...
    'SUBSTRING', 'STR_POSITION', 'IF', 'DATE', 'CURRENT_DATE', 'CASE', 'AND', 'OR', 'EXISTS',
}
...
def validate_sql(sql: str, allowed_tables=None, max_chars=20_000) -> ValidatedSQL:
    ...
    if allowed_tables is not None and physical - (set(allowed_tables) | available_views(allowed_tables)):
        raise SchemaMismatch(physical - (set(allowed_tables) | available_views(allowed_tables)))
