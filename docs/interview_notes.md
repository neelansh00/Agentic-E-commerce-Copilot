...
## Phase 4.5: what the correction pass changed

**Why shared metric views?** The model was repeatedly reconstructing payment aggregation, review selection and delivery eligibility. Four generic views make those reusable calculations explicit at order, order/category, order/seller and month grains. This is a small semantic layer, not a lookup of question-specific SQL: the model still generates grouping, filters and analysis. Alternatives were longer prompts or individual query templates. Views reduce repeated correctness-sensitive logic but add database-specific SQL and can cost execution time. At scale use versioned warehouse models, reviewed metric contracts and measured materialization choices.

**Why a calendar spine and nullable measures?** Grouping observed orders cannot return an empty month. A continuous month view joined to observations preserves the time axis. Missing payments are unknown, while recorded zero payments are observations; coercing both to zero conceals data coverage. The alternative is sparse series with explicit missing-period display, appropriate when continuity is not requested. Boundary months are flagged rather than presented as complete. At scale a governed date dimension should incorporate fiscal calendars and explicit timezone policy.

**Why retrieve definition prerequisites?** A seller-performance paragraph depends on successful-order, review and delivery definitions. Semantic top-k alone may miss these prerequisites. The SQL retrieval path expands explicit dependencies transitively while preserving source citations. This is explainable for thirteen chunks, but may grow prompts or require maintenance. Larger knowledge bases need versioned dependency metadata, token budgets and evaluation of completeness as well as relevance.

**Why stop rendering model labels?** A copied value is still misleading if called highest or assigned to a different seller. The model now selects cells, while deterministic captions and same-row identities describe them. This removes unsupported extrema assertions and retains invalid-reference rejection. The trade-off is limited prose and incomplete summaries; five of twenty-two corrected answers still fall back to literal evidence. An alternative is a richer typed claim language with separately verified comparisons, which should be added only with evaluation. Production systems need independent review and meaningful question-coverage checks, not just valid cell references.

**What can I claim from the results?** All 99 offline tests, sixteen full-data checks and eleven reference replays passed at Phase 4.5. Both corrected live context modes matched 11/11 complete reference results, up from 8/11. These are the same development questions that motivated the fixes, not held-out accuracy. One rerun per context and several simultaneous changes cannot establish which component caused improvement. No misleading rendered interpretation was found during coding-assistant review; that is not independent human certification or proof of fluent generative answer quality. Preserve this distinction in resume bullets. Later phases and the broader benchmark are documented below.


## Phase 5: why this agent is intentionally bounded

**Why call this an agent instead of a deterministic dashboard?** One orchestrator chooses among tools from natural-language intent and delegates arbitrary supported business SQL generation to the LLM. The tool selection itself is deterministic, and known statistical extraction is fixed SQL. It is a bounded analytics assistant, not an autonomous planner. A completely fixed dashboard cannot answer new SQL questions; a fully model-driven loop adds latency, cost and failure modes. Add structured model routing only if a separate paraphrase evaluation demonstrates that these explicit rules are insufficient.

**Why one Python operation?** The first useful case is late delivery versus review scores. SQL retrieves a complete two-by-five histogram at verified order grain; Python computes means and cross-group pair proportions. This isolates statistical reasoning from database retrieval and avoids transferring individual reviews or generating arbitrary code. SQL could express the pair calculation, but the small Python function makes its ordinal interpretation and tests easier to inspect. The cost is deliberately narrow scope: filtered comparisons, regression, forecasting and significance tests are unsupported. At production scale introduce typed operation-specific filters, approved cohorts and additional independently tested functions rather than an unrestricted Python interpreter.

**Why not run a t-test automatically?** The observed snapshot is not a randomized experiment. Review scores are ordinal, missing reviews can select the population, and orders share customers/sellers. A p-value would require a defensible sampling/dependence model. Descriptive mean differences and ordinal pair proportions answer the observed association question without a significance claim. At scale investigate clustered inference or modeling only after specifying the estimand and assumptions.

**Why no extra LLM answer call for statistics?** All values and captions are deterministic functions of the complete histogram. This keeps numerical claims auditable and avoids the label problems found in Phase 4.5. The disadvantage is less flexible prose. SQL answers retain their existing constrained model cell selection; definition answers remain cited excerpts. These are different modes and are labeled through the trace.

**What did we actually measure?** The Phase 5 report distinguishes 29 same-author routing development cases, full-data histogram agreement, synthetic failures and two live SQL smoke questions. All 114 tests pass in the final offline run. The two live results are reused from the preceding run after only a test-fixture clarification and reporting changes; no new live sample is implied. The earlier eleven-question Phase 4.5 benchmark is historical evidence, not a new agent evaluation. At scale add held-out paraphrases, filtered requests, adversarial routing and coverage/abstention metrics before claiming broader language understanding.


## Phase 6: interface decisions

**Why Streamlit?** The product is conversational analysis with inspectable tables and charts. Python-native widgets let us reuse the tested agent without building a second backend and a JavaScript frontend. The downside is rerun semantics and less UI/control flexibility. At production scale, session capacity, resource isolation, authentication and long-running jobs need explicit design; a separate web client is an option only if those needs justify it.

**Why keep the UI thin?** A service function performs setup and calls `run_agent`; presentation functions only format validated results. SQL, business metrics and retry rules do not live inside widgets. This enables Streamlit AppTest plus unit tests without requiring network calls. We considered putting routing and SQL directly into the page, but that would duplicate the correctness boundaries already tested in earlier phases.

**How do reruns avoid repeated model calls?** Only a submitted chat input or example click calls the service. The typed result is saved in session history, and reruns render that saved result. Model clients are request-local; the local retriever is cached per session. A global mutable model or shared history could mix usage or results across users. The trade-off is additional per-session embedding memory; production would need measured concurrency and thread-safe resource pooling.

**Why conservative automatic charts?** A valid query can return a shape that is not meaningful to plot automatically. Charts require known measures, unique groups and an untruncated result. Raw cents become BRL only in chart display. Missing values remain NULL and monthly lines break across gaps, so the picture does not imply fabricated continuity. Ambiguous results remain tables. A future richer chart planner should have its own correctness evaluation.

**Why no conversational memory yet?** Displayed chat history helps a demo, but resolving “those sellers” changes query scope. The Phase 5 router intentionally asks users to restate the question. Adding hidden history to prompts now would bypass that design and require new coreference/scope tests. At scale, explicit typed filters and confirmed conversation context would be preferable to concatenating every prior message.

**What was measured?** Phase 6 has 128 passing tests, including fourteen new tests covering UI reruns, isolation, clear/history bounds, failure/truncation display, chart units and NULL/gap handling. Four real-data AppTest flows passed, including one live count query. A real browser confirmed the statistical chart and SQL/data panels; screenshots are checked in. These are functional checks, not a usability study, load test or additional model-accuracy benchmark.

## Phase 7: evaluating the system honestly

**Why independent CSV ground truth?** Comparing generated SQL with another SQL query is useful, but both can share a wrong join. All thirty quantitative reference queries were checked against raw-CSV calculations before freezing fifty cases. The trade-off is duplicated metric logic to maintain. Production evaluation needs versioned data/definitions, external review and representative held-out requests. Explicit output-column contracts simplify comparison here and must be disclosed.

**Why distinguish execution from answer accuracy?** A valid query can calculate the wrong population or even a different metric. Our agent achieved 28/30 intended SQL tasks executing and matching reference values, but only 47/50 total tasks succeeded. Two legitimate seller questions were blocked by broad follow-up routing rules; a payment-revenue question was changed to item sales after retry feedback. Neither a passing parser nor copied numerical cells detects that change of intent. Production must preserve a typed requested metric and treat missing allocation policy as clarification.

**Why evaluate refusals?** Answering everything creates fabricated analytics when costs, causal evidence or an allocation rule are absent. Correct abstentions are valid outcomes, but broad refusal rules also reject answerable questions. Denominators include these failures. The groundedness review counts substantive answers separately so abstention cannot inflate numerical answer support.

**Did schema retrieval improve accuracy?** Not in this ten-question comparison: full schema matched 9/10, retrieved schema 8/10; both executed 9/10. Retrieved context used 17.54% fewer input tokens. Keep the simple keyword implementation and full-schema evaluation alternative; do not add embedding routing or claim an accuracy improvement without broader evidence. One stochastic sample per arm cannot settle the best default.

**Did validation and retry help?** With the exact same starting SQL, the guarded arm repaired one invalid state-column query: execution improved from 9/10 to 10/10 and exact results from 8/10 to 9/10. The remaining monthly query executed but had NULL coverage counts for an empty month. A runtime error loop cannot fix undetected semantics. The baseline ran without application validation on an isolated copy, retaining SQLite authorization, read-only settings and resource limits. Never remove all safety to perform this experiment.

**Did RAG help generated definitions?** Required project-fact coverage rose from 0/8 to 6/8 on assistant semantic review. Without context, generic definitions or honest uncertainty omitted project policy; one answer used customer-acquisition cohorts instead of order-purchase cohorts. All eight RAG answers passed lexical patterns, but two omitted a required NULL/eligibility detail. This is why keyword coverage is a proxy, not proof of complete grounded answers. The main agent's definition tool returns cited text directly; this experiment deliberately compares generated prose in both arms.

**Why no model judge as the only ground truth?** A second model can agree with an incorrect query or overlook negation. We used exact numeric comparisons, retrieval-heading checks and explicit assistant review of facts and caveats. That review is not independent human annotation, and summary completeness remains a separate weakness. Production needs a blinded human-labeled set, multiple reviewers where justified, repeated trials and confidence intervals.

**Which resume claims are defensible?** A fifty-question development evaluation; 28/30 SQL reference matches; 48/50 tool-selection matches; 137 offline tests; measured 17.54% schema input-token reduction; one successful bounded repair. State the development-set scope and denominators. Do not claim 100% reliable business answers, a statistically proven latency gain, real business impact or held-out generalization. The documented failures are useful interview evidence about system boundaries, not results to hide.

## Phase 8: packaging and explaining the project

**Updated delivery choice:** Docker was removed at the user's request after the local engine failed to start. The primary setup is now a Python virtual environment with a Windows launcher. Streamlit Community Cloud and Railway have separate managed deployment instructions and a hosted entry point. The Docker rationale below records the earlier option, not a current requirement.

**Why a runtime asset bundle?** Git should not hold credentials, raw CSVs or large rebuildable model files. A ten-file bundle ships the exact database/model/index snapshot used locally. The host verifies a separately configured SHA-256 and safely restores it before use. This adds cold-start download time and an external artifact URL to maintain. Alternatives are ingesting raw CSVs at startup, Git LFS or a hosted database; the snapshot keeps this read-only portfolio demo simpler. Production would use governed versioned storage, retention/access policies and managed database services where concurrency requires them. No actual cloud deployment is claimed from local hosted-entry tests.

**Why Docker as an option alongside a virtual environment?** A small Linux image packages the Streamlit runtime without requiring the reviewer to configure Python. A virtual environment is easier for learning, debugging and running ingestion. Alternatives include shipping only requirements or a hosted application. Docker adds a daemon and image maintenance, so it is optional. At production scale pin image digests, lock transitive dependencies, scan and rebuild images, and validate the target platform. Direct package pins alone do not guarantee identical builds.

**Why keep data and weights outside the image?** Source CSVs and model files are large, rebuildable artifacts; credentials must never be baked into a distributable image. The build context is an allowlist, and Compose mounts prepared data read-only while passing only the two optional model settings. The trade-off is a preparation step and host-file permissions. Baking public model weights into a versioned image could simplify distribution at a size cost; production would use governed artifact storage and secret injection. A local `.env` is convenience, not a multi-user secret-management solution.

**Why a non-root process and localhost binding?** The application needs only read access to prepared analytics assets and an HTTP port. These inexpensive defaults reduce unnecessary access. They are not authentication or a guarantee against every attack. A shared deployment would need identity, authorization over datasets, request budgets, logging/retention policy and a deliberate privacy review; none is claimed here.

**Why retain the Phase 7 failures in a polished portfolio?** Packaging and good documentation do not improve result accuracy. The current code still has two seller-routing false positives and a payment-to-item-sales substitution failure. Keeping frozen evidence gives a reviewer a truthful account of the system. The next correction should add general regression cases, preserve metric intent across retries and rerun the same benchmark plus held-out paraphrases. Replacing old results with favorable reruns would conceal development-set overfitting.

**How do I explain the project in ninety seconds?** “I built an analytics assistant over nine Olist tables. One orchestrator routes definitions to local retrieval, business queries to guarded model-generated SQL, and one statistical comparison to a restricted Python function. Shared metric views preserve grain, NULLs and delivery eligibility. Results and sources are visible. I evaluated fifty development questions: 47 succeeded, and the three failures exposed routing and metric-intent weaknesses. The project demonstrates both measurable functionality and the limits of SQL safety as a correctness guarantee.” Then draw the architecture and walk through a query, not a list of framework names.
